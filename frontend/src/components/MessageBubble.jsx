...
export default function MessageBubble({ message, isOwn, onEdit, onDelete, onForward }) {
  ...
  return (
    <div className={`flex ${isOwn ? "justify-end" : "justify-start"} mb-1 group`}>
      {/* Wrapper with relative positioning for the context menu (no overflow-hidden) */}
      <div className="relative max-w-[70%]">
        <div
          className={`${hasImage ? "p-1" : "px-3 py-2"} rounded-2xl text-sm leading-relaxed overflow-hidden ${
            isOwn
        ...
        >
          ...
          {message.forwarded_from && (
            <div className={`text-tg-muted text-xs italic mb-0.5 ${hasImage ? "px-2 pt-1" : ""}`}>
              ↪ Forwarded from {sanitizeText(message.forwarded_from.display_name || message.forwarded_from.phone)}
            </div>
          )}
        ...
        {/* Context menu — outside overflow-hidden, inside relative wrapper */}
        {showMenu && (
          <div
            ref={menuRef}
            className="absolute bottom-full right-0 mb-1 bg-tg-sidebar border border-gray-600 rounded-lg shadow-xl z-50 overflow-hidden min-w-[140px]"
          >
